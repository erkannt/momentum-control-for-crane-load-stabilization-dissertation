...
By delving into the various existing applications, this chapter hopes to provide a deeper understanding of the potentials and limitations of CMGs, to thereby motivate the subsequent work.
...
This happens when a motor spins a wheel or we snap our fingers to spin a top.
...
Before proceeding, I would like to emphasize three key takeaways:
...
Since the gyroscope's motor only needs to overcome the bearing friction, once it has reached the desired speed, CMGs let us produce very large torques with comparatively small motors.
...
In such a case the angular momentum lever becomes a problem, as it now leads to a large torque being put on our gimbal motor.
Sizing the motor to accommodate for such cases can quickly negate the advantage provided by a CMG and this will be discussed in the later sizing sections.
...
This explains why momentum control devices mostly associated with spaceflight.
...
Reaction wheels consist of a single motor attached to disc or wheel.
...
Dual gimbal CMGs are less efficient than single gimbal CMGs, as the produced torque is always transferred in part through a gimbal motor, whereas in a single gimbal design it will be transferred via the bearings.
But there are several reasons why Skylab and many subsequent spacecraft including the ISS use dual gimbal CMGs.
...
The Apollo program did not use CMGs, but did use three gyroscopes for inertial measurement of the spacecrafts orientation.
These gyroscopes where suspended in three nested gimbals, providing feedback regarding the spacecraft's yaw, pitch and roll.
The decision to only use three gimbals lead to more frequent issues with gimbal lock (a singularity where the gimbal axis align) than was expected during multiple Apollo missions[^collinsxmas].
...
[^collinsxmas]: The Apollo guidance computer had to include functions to alert the astronauts of an impending gimbal lock and Mike Collins joked that all he wanted for Christmas was a fourth gimbal as he was repeatedly resetting the gimbals during Apollo 11. 

The attitude control systems for the first spacewalks of the US program took a different approach.
By mechanically linking two CMGs so that they mirror each others motion, one can create a scissored pair CMG array.
...
The downside is that one has to use six, instead of three CMGs to be able to produce torque around three axis (i.e. three degrees of rotational freedom).
...
Since redundancy is a must in spaceflight, the arrays usually use four CMGs, unless other momentum control devices are also included in the spacecraft.
...
But these systems also come into play when one attempts to add robot arms to spacecraft.
...
Note that such speeds are orders of magnitude below those of earth bound industrial robots.
...
Attitude control with CMGs makes little sense on earth, as most vehicles need to rotate at velocities that aren't feasible for CMG based control, given the high reaction torques that would have to be sustained by the gimbal motors.
...
Given that control engineering or cybernetics hadn't come about yet, these systems must have operated on a passive control principle.
...
The low speeds of the motors available back then as well as the much simpler solution of adding further wheels or putting a foot on the ground are why gyroscopically stabilized ground vehicles didn't take off in the early 20th century.
...
The actuation is required to ensure that the gyroscope doesn't reach a singularity and also to limit the precession speed and thereby the output torque.
...
This is relevant during small excitations, where the friction might introduce a significant time delay in the systems reaction.

Researchers have also proposed other applications for CMGs (see @Fig:other-terrestrial).
The most exotic is likely the use of a CMG to stabilize a beating heart during surgery [@GagneCardiacMotionCompensation2009; @GagneGyroLockStabilizingHeart2012].
...
Given that there have been significant efforts to automate construction in Japan and Korea dating back to the 1980s, it is unsurprising that one can find research into the actuation of cranes from these countries.
A work from 1994 is relevant to cranes, but actually on the stabilization of a passenger gondala.
...
The authors run into issues with the singularity and changing orientation of the output torque.
...
The trajectory optimizations proposed for space robotics can most likely be adapted to our usecase, but can't be applied directly due to the stark differences between spacecraft and crane dynamics.
...
We will have to develop an understanding of crane-CMG dynamics to let us find a way to consolidate the different control and sizing requirements.
...
Leve et al. use vector-dyadic notation for the above equations and spend a section introducing it in [@LeveSpacecraftMomentumControl2015, sec. 4.1].
...
We can think of this as the workspace of the array whose axes are angular momentum around the three cartesian axes.
...
The workspace enclosed by the envelope then is the set of points defined by the sum of multiple such vectors as they are rotated.
...
This is akin to robots axis having to rotate further achieve a given travel in cartesian space when they are close to singularities.
...
Leve et al. also include a more in depth discussion of the various types of singularities and how the different steering laws take them into account.
...
The shape of the workspace is of some relevance to us as our requirements regarding it won't be uniform in all directions.
