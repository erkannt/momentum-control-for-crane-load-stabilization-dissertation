...
To develop sizing guidelines we need to develop two types of models that enable us to do this:
...
Overall these models should thereby provide us with the means to abstract crane parameters to assist in the sizing of the CMGs.
...
Then, to link crane and application, we require a suitable model of the CMG system.
...
To understand the interaction of a CMG array with a crane, we will model it as a double pendulum system, apply the formulation of CMG dynamics (see @Sec:cmg-dynamics) to it and create interfaces to apply external forces.

## Parameter Space of Construction Cranes { #sec:crane-params }

Given the construction focus of this work, we will look at tower cranes for the crane parameter space.
...
We are predominantly interested in the parameters that enable us to estimate the base rate of the CMGs induced by the motion of the crane, as this determines the magnitude of the gyroscopic reaction torque that our gimbal motors need to compensate (see @Sec:cmg-dynamics).
The translational movements of the hook/platform do not contribute to the base rate, but, as will be mentioned again later, the period of the crane-rope/pendulum is of interest.
...
Now whilst neither the amplitude of the oscillation nor the mass of the pendulum affect the frequency of the oscillation, they do affect the maximum angular velocity.
...
This mixed approach of a SPCMG and 2D crane can be continued into the production of a first hardware prototype.
By building the crane as a swing that hangs from two diagonally spanned ropes, the motion to a single plane can be roughly constrained (see @Fig:prototype-sketch).
This reduces the hardware investment for the first prototype by requiring two instead of four CMGs and still allows CMG behavior to be validated.
It also limits the number of components that could cause difficulties during the development of the controller, sensor and communication systems.
...
Since no model can fully encompass reality one has to determine where one can simplify and where one can not.
...
It is should be possible to copy, mix and adapt these to create a model suited for the current purpose.
...
It is my hope that this extended discussion and availability of source code will aid others in extending the model to their needs, especially when coming from other disciplines.
...
It is disappointing that the use of spherical coordinates to describe the kinematic constraints of the system leads to numerical issues during simulation.
...
The following extends this to a full three degrees of freedom (see @Fig:crane-projected-angles), not only to model realistic crane load motion more closely but also to accommodate our use-case of load rotation.
...
![Sequence of rotations to transform the world aligned reference frame to the reference frame of the distributed mass.](./figures/cog-rotations.png){ #fig:cog-rotations }
...
In the following section it will be seen that certain aspects can be added relatively easily by modifying the state of the system.
...
![Rudimentary implementation of this model in Modelica. Note that this lacks connections to input torque from the CMGs and uses a different angle description.](./figures/modelica-example.gif){ #fig:modelica-example }
...
Note how this encompasses _all_ torques produced by the CMG, thereby cleanly separating the crane and CMG models and simplifying their respective development.
...
As the pivot of the upper link is assumed to be fixed and is realistically speaking connected by a rope, the translational acceleration has no impact.
...
![Example payload inertia assuming constant density (2,400 kg/m$^3$), proportions (5, 0.1, 2) and a weight stemming from the max load of the associated crane. It is also assumed that the CoG is to be offset by the Z-size from the point of rotation.](./figures/inertia-data.png){ #fig:inertia-data }

## Process Torques and Forces

Given the initial motivation of this work to stabilize a robot hanging from a crane, a robot is used as the model for a process generating torques and forces to be compensated by the CMGs.
...
The inertia of the robot's axes are estimated by distributing the robot’s mass according the volume of each axis, which assumes a homogeneous density of the robot.

![Three robot paths used in these experiments.](./figures/robot-toolpaths.gif){#fig:robot-toolpaths}
