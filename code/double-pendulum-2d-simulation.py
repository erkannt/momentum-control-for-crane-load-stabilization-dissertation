...
def deriv_cas(y, t, l1, l2, m1, m2):
    ...
    theta1dot = z1
    theta2dot = z2
    z1dot = -(g*m1*sin(theta1) + g*m2*sin(theta1 - 2*theta2)/2 + g*m2*sin(theta1)/2 + l1*m2*z1**2*sin(2*theta1 - 2*theta2)/2 + l2*m2*z2**2*sin(theta1 - theta2))/(l1*(m1 - m2*cos(theta1 - theta2)**2 + m2))
    z2dot = (g*m1*sin(2*theta1 - theta2)/2 - g*m1*sin(theta2)/2 + g*m2*sin(2*theta1 - theta2)/2 - g*m2*sin(theta2)/2 + l1*m1*z1**2*sin(theta1 - theta2) + l1*m2*z1**2*sin(theta1 - theta2) + l2*m2*z2**2*sin(2*theta1 - 2*theta2)/2)/(l2*(m1 - m2*cos(theta1 - theta2)**2 + m2))
    return theta1dot, z1dot, theta2dot, z2dot
...
def plot_motion(t, y, title="", save=False):
  ...
  ani = animation.FuncAnimation(fig, animation_func, range(0, t.size, di),
                                interval=fps, blit=True, init_func=animation_init)

  #Later ffmpeg crop=1246:646:352:560 to limit to actual motion
  if save:
    ani.save('../output/double_pendulum_2d_%s.mp4' % title,
...
np.savez('../output/pointmass-double-pendulum-large_exitation-2d-literature_eom',
         t=t, y0=y0, y=y)
np.savez('../output/pointmass-double-pendulum-large_exitation-2d',
         t=t, y0=y0, y=y_cas)
np.savez('../output/pointmass-double-pendulum-small_exitation-2d',
         t=t, y0=y0_small, y=y_cas_small)
```
