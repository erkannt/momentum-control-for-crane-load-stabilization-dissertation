...
def deriv(t, y, l1, l2, m1, m2):
    ...
    theta21, u21, theta22, u22, theta23, u23 = y
...
def plot_motion(t, y, title="", save=False):
  ...
  if save:
    ani.save('../output/double_pendulum_3d_distmass_%s.mp4' \
...
def plot_angles(time, y, events):
  ...
  linetypes = ['-', '--', '-.', ':']
  labels = {
    '3d': [r'$\theta_{11}$', r'$\theta_{12}$',
           r'$\theta_{21}$', r'$\theta_{22}$', r'$\theta_{23}$'],
    '3ddot': [r'$\dot{\theta}_{11}$', r'$\dot{\theta}_{12}$',
              r'$\dot{\theta}_{21}$', r'$\dot{\theta}_{22}$',
              r'$\dot{\theta}_{23}$']
  }
  ...
  for i, thetaid in zip([0, 1], [0, 1]):
    axs[0].plot(time, y[thetaid*2, :] * 180/np.pi,
      linetypes[i],
      color=colors[i],
      label=labels['3d'][thetaid],
      linewidth='0.85')
  for i, thetaid in zip([0, 1, 2], [2, 3, 4]):
    axs[1].plot(time, y[thetaid*2, :] * 180/np.pi,
      linetypes[i],
      color=colors[i],
      label=labels['3d'][thetaid],
      linewidth='0.85')
...
def save2file(y, filename):
  ...
  cogxx = -np.sin(theta21)*np.sin(theta22)*np.sin(theta23) + np.cos(theta21)*np.cos(theta23)
  cogxy = np.sin(theta23)*np.cos(theta22)
  cogxz = np.sin(theta21)*np.cos(theta23) + np.sin(theta22)*np.sin(theta23)*np.cos(theta21)
  cogyx = -np.sin(theta21)*np.sin(theta22)*np.cos(theta23) - np.sin(theta23)*np.cos(theta21)
  cogyy = np.cos(theta22)*np.cos(theta23)
  cogyz = -np.sin(theta21)*np.sin(theta23) + np.sin(theta22)*np.cos(theta21)*np.cos(theta23)
