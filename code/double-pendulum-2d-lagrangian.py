...
q = [theta1, theta2]
substitutions = [(theta1.diff(t).diff(t), zdot1),
...
""" Langrangian """
Pot = m1 * g * y1 + m2 * g * y2
Kin = ( 1/2 * m1 * (x1.diff(t)**2 + y1.diff(t)**2)
      + 1/2 * m2 * (x2.diff(t)**2 + y2.diff(t)**2))
L = Kin - Pot

""" Validate against known Lagrangian """
testPot = (m1 + m2) * l1 * -g * cos(theta1) - m2 * l2 * g * cos(theta2)
testKin = (0.5 * m1 * l1 * l1 * z1 * z1 + 
          0.5 * m2 * (l1 * l1 * z1 * z1 + l2 * l2 * z2 * z2 + 2 * l1 * l2 * z1 * z2  * cos(theta1 - theta2)))
