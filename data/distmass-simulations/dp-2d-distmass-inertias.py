...
labels2d = [r'$\theta_{1} [^\circ]$', r'$\dot{\theta}_{1} [\frac{rad}{s}]$', r'$\theta_{2} [^\circ]$', r'$\dot{\theta}_{2} [\frac{rad}{s}]$']
...
def deriv(t, y, l1, l2, m1, m2, I2):
    ...
    theta1dot = z1
    theta2dot = z2
    z1dot = 1.0*(-l2**2*m2**2*(g*sin(theta2) - l1*z1**2*sin(theta1 - theta2))*cos(theta1 - theta2) + (I2 + l2**2*m2)*(g*m1*sin(theta1) + g*m2*sin(theta1) + l2*m2*z2**2*sin(theta1 - theta2)))/(l1*(l2**2*m2**2*cos(theta1 - theta2)**2 - (I2 + l2**2*m2)*(m1 + m2)))
    z2dot = 1.0*l2*m2*((m1 + m2)*(g*sin(theta2) - l1*z1**2*sin(theta1 - theta2)) - (g*m1*sin(theta1) + g*m2*sin(theta1) + l2*m2*z2**2*sin(theta1 - theta2))*cos(theta1 - theta2))/(l2**2*m2**2*cos(theta1 - theta2)**2 - (I2 + l2**2*m2)*(m1 + m2))
...
def plot_motion(t, y, title="", save=False):
    ...
    pltsize = 1.2 * (l1 + l2)
    ax = fig.add_subplot(
        111, autoscale_on=False, ylim=(-pltsize, 0.05*pltsize), xlim=(-pltsize*0.5, pltsize*0.5)
    )
...
zero_inertia = solve_ivp(lambda t, y: deriv(t, y, l1, l2, m1, m2[0], 0), (0, tmax), y0_small, t_eval=t)
solutions = [zero_inertia]

for m2i in m2:
  I2 = 1/2 * m2i * r2**2 + m2i * l2**2
  sol = solve_ivp(lambda t, y: deriv(t, y, l1, l2, m1, m2i, I2), (0, tmax), y0_small, t_eval=t)
  solutions.append(sol)
data = [sol.y for sol in solutions]

if not output:
  save_animations = False
  plot_motion(t, data[0], title="Point Mass (no rotational inertia)", save=save_animations)
